...
**Note:** This change alters the bucket boundaries of the `scaffolder.task.duration` and `scaffolder.step.duration` OpenTelemetry histogram metrics. If you have dashboards or alerts that depend on specific bucket label values (e.g. `le="5"`) for these metrics, they will need to be updated.
